    ...
    for src in sources:
        ...
        try:
            if src.source_type == "rss":
                ...
                if proxy_url:
                    transport = httpx.HTTPTransport(proxy=proxy_url)
                with httpx.Client(timeout=settings.source_connect_timeout_seconds, transport=transport) as client:
...
#
def ingest_sources(db: Session) -> dict:
    sources = db.scalars(select(Source).where(Source.enabled_default.is_(True))).all()
    proxy_url = _runtime_proxy_url(db)
    if not sources:
    ...
    return {"sources": len(sources), "fetched": fetched, "inserted": inserted}
